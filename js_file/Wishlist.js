...
setInterval(() => {
    ...
    if (currentImage === 1) {
        img.src = "../image/fav2.png";
        currentImage = 2;
    } else {
        img.src = "../image/favi.png";
        currentImage = 1;
    }
}, 3000); // كل 2 ثانية
...
// الوصول إلى الجسم الخاص بالجدول
const tableBody = document.querySelector('#wishlist-table tbody');

// دالة لعرض المنتجات في الجدول
function displayWishlist() {
    // تفريغ محتوى الجدول قبل إضافته مرة أخرى
    tableBody.innerHTML = '';

    // تكرار المنتجات في المفضلة وعرضها
    wishlist.forEach((product, index) => {
        ...
        row.innerHTML = `
          ...
          <td>${product.name}</td>
          <td>${product.price}</td>
          <td><span class="remove" data-index="${index}">×</span></td>
        `;
...
function removeFromWishlist(index) {
    ...
    // إعادة عرض المفضلة بعد الحذف
    displayWishlist();
}

// إضافة مستمعات الحدث على أزرار الحذف
