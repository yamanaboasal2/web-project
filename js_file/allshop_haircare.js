...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function collectStaticProducts() {
        ...
        productCards.forEach((card, index) => {
            ...
            staticProducts.push({
                id: `static-${index + 1}`,
                name,
                price: parseFloat(priceText) || 0, // تحويل إلى عدد مع قيمة افتراضية
    ...
    function loadDynamicProducts() {
        fetch('../php/get_products.php?type=haircare')
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                // دمج المنتجات اليدوية والديناميكية
            ...
            .catch(error => {
                ...
                // عرض المنتجات اليدوية فقط في حالة الخطأ
    ...
    function applyFiltersAndSort() {
        ...
        switch (currentSort) {
            ...
            case 'date-new':
                ...
                break;
            default:
                // featured (افتراضي): المنتجات اليدوية أولاً
    ...
    // تأثير الظهور لـ haircare-intro-content
    const introContent = document.querySelector('.haircare-intro-content');
    if (introContent) {
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
